...
class Model {
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        if(false !== $result && is_numeric($result)) {
            ...
            if($insertId) {
                ...
                // 记录表操作的历史记录 操作成功之后再去添加操作记录
                if($result != 0 && $tableName != 'crm_stafflog' && $this->name != ''){
                    $logRes = $this->_addTableAddLog($options, 1, $pk, $insertId);
                    if(!$logRes){
                        return false;
                    }
                }
    ...
    public function addAll($dataList,$options=array(),$replace=false){
        ...
        if(false !== $result ) {
            ...
            if($insertId) {
                if (!empty($_SESSION["table_log_add"])){
                    return $insertId;
                }
                // 记录表操作的历史记录 操作成功之后再去添加操作记录
                $tableName = $options['table'];
                if($result != 0 && $tableName != 'crm_stafflog' && $this->name != ''){
                    $logRes = $this->_addTableAddLog($options, $result, $pk, $insertId);
                    if(!$logRes){
                        return false;
                    }
                }
     ...
     * @param $num 新增条数
     * @param $pk 主键字段
     * @param $insertId 最后新增的那条数据的主键
     * @return bool
     */
    protected function _addTableAddLog($options, $num, $pk, $insertId){

        // 操作的controller 和 action
        $path = $_SERVER['PATH_INFO'];

        // 表名
        $tableName = array_filter(explode(' ',$options['table']))[0];

        // 标志当前是新增操作
        $type = 1;

        // 获取表字段的注释
        $tableData = M()->query("show full fields from $tableName");
        $commentMap = str_replace("'", '"', json_encode(array_column($tableData,'comment','field'), JSON_UNESCAPED_UNICODE));

        $map[$pk] = ['elt', $insertId];
        $newData = M()->table($options['table'])->where($map)->limit($num)->order($pk . " desc ")->select();
        // 主键的值
        $majorKey = implode(',',array_column($newData, $pk));

        // 修改后数据转化为json
        $newData = str_replace("'", '"', json_encode($newData,JSON_UNESCAPED_UNICODE));
        if (session('staffId')) {
            // 插入log记录
            $sql = sprintf("INSERT INTO `manage`.`crm_table_log` (
                                `staff_id`,
                                `staff_name`,
                                `path`,
                                `table_name`,
                            ...
                            VALUES
                                (
                                    ...
                                    '%s',
                                    %s
                                );", session("staffId"), session("nickname"), $path, $tableName, $commentMap, $type, $majorKey, $newData, time());
    ...
    public function save($data='',$options=array()) {
        ...
        // 查询更新之前的数据
        $oldData = M()->table($options['table'])->where($options['where'])->select();
        ...
        if(false !== $result && is_numeric($result)) {
            ...
            $tableName = $options['table'];
            if($result != 0 && $tableName != 'crm_stafflog' && $this->name != ''){
                $logRes = $this->_addTableUpdateLog($data, $options, $oldData, $pk);
                if(!$logRes){
                    return false;
                }
            }
     ...
     * 添加表数据操作的历史记录
     * @param $data 需要操作的数据
     * @param $options 操作的表信息和条件之类的
     * @param $oldData 操作之前的数据
     * @param $pk 主键字段
     * @return bool
     */
    protected function _addTableUpdateLog($data, $options, $oldData, $pk){
        // 操作的controller 和 action
        $path = $_SERVER['PATH_INFO'];
        ...
        foreach ($oldData as $k => $v){
            $newData[] = array_merge(array_diff_assoc($data, $v),array_diff_key($v,array_diff_assoc($data, $v)));
        }

        $majorKey = implode(',',array_column($oldData, $pk)); // 所修改数据的主键

        // 修改前数据转化为json
        $oldData = str_replace("'", '"', json_encode($oldData,JSON_UNESCAPED_UNICODE));
            ...
            $sql = sprintf("INSERT INTO `manage`.`crm_table_log` (
                            ...
                            VALUES
                                (
                                    ...
                                    '%s',
                                    %s
                                );", session("staffId"), session("nickname"), $path, $tableName, $commentMap, $type, $majorKey, $oldData, $newData, time());
            $res = $this->execute($sql);
            if(!$res){
                return false;
            }
